class /ADZ/SCI_TEST_WUL definition
  public
  inheriting from CL_CI_TEST_ROOT
  create public .

*"* public components of class /ADZ/SCI_TEST_WUL
...
public section.
  ...
  methods RUN
    redefinition .
protected section.
*"* protected components of class /ADZ/SCI_TEST_WUL
*"* do not include other source files here!!!
private section.
*"* private components of class /ADZ/SCI_TEST_WUL
*"* do not include other source files here!!!

  constants C_MY_NAME type SEOCLSNAME value '/ADZ/SCI_TEST_WUL' ##NO_TEXT.
ENDCLASS.



CLASS /ADZ/SCI_TEST_WUL IMPLEMENTATION.
  ...
  METHOD CONSTRUCTOR .
    ...
    category            = '/ADZ/SCI_CATEGORY_ADZ'.  "required
    version             = '000'.
  ...
  METHOD GET_RESULT_NODE.
    ...
    CREATE OBJECT p_result TYPE /adz/sci_result_wul
      EXPORTING
        p_kind = p_kind.
...
*    Quelle: CL_MDG_GN_BBI->GET_CROSSREF
    DATA: lt_sciobj TYPE TABLE OF /adz/sci_objects.
    FIELD-SYMBOLS: <ls_sciobj> TYPE /adz/sci_objects.
    ...
    SELECT * FROM /adz/sci_objects INTO TABLE lt_sciobj.
    IF sy-subrc <> 0.
