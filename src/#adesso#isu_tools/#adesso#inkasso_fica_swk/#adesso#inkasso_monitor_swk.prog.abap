...
*&
*&---------------------------------------------------------------------*
REPORT /adesso/inkasso_monitor_swk.
...
* Positionstabelle
DATA: pos_itab_marked TYPE TABLE OF /adesso/inkasso_out_swk WITH HEADER LINE,
...
DATA: wa_tout TYPE  /adesso/inkasso_out_swk.
DATA: ft_out TYPE TABLE OF /adesso/inkasso_out_swk.
DATA: t_out  TYPE TABLE OF /adesso/inkasso_out_swk.
...
START-OF-SELECTION.
  ...
  LOOP AT t_tasks ASSIGNING <t_tasks>.
    ...
    CALL FUNCTION '/ADESSO/INKASSO_SELECT_SWK'
...
* Geschäftspartner-Name
  ...
  ls_fieldcat-tabname = 'T_OUT'.
  ls_fieldcat-seltext_s = 'Name GP'.
  ls_fieldcat-seltext_m = 'Name GPartner'.
  ls_fieldcat-seltext_l = 'Name Geschäftspartner'.
  APPEND ls_fieldcat TO lt_fieldcat.
...
*   Wenn Kunde zur Abgabe gesperrt ist, kann er nicht freigegeben werden
    IF wa_out-locked IS NOT INITIAL.
      MESSAGE e000(e4) WITH 'Kunde' wa_out-gpart 'kann nicht freigegeben werden - ist gesperrt'.
      CLEAR wa_out.
      EXIT.
    ENDIF.
...
FORM freigabe .
  ...
  DATA: itab_release               LIKE TABLE OF fkkop WITH HEADER LINE,
        ...
        ht_pos                     TYPE /adesso/inkasso_out_swk OCCURS 0 WITH HEADER LINE,
