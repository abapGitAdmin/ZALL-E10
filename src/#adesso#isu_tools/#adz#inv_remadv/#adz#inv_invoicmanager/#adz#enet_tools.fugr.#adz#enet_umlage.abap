...
*"  EXPORTING
*"     REFERENCE(SKAP) TYPE  /ADZ/ENET_PREISE_T
*"     REFERENCE(SKAPP) TYPE  /ADZ/ENET_PREISE_T
*"     REFERENCE(SKU_A) TYPE  /ADZ/ENET_PREISE_T
*"     REFERENCE(SKU_B) TYPE  /ADZ/ENET_PREISE_T
*"     REFERENCE(SKU_C) TYPE  /ADZ/ENET_PREISE_T
*"     REFERENCE(OFF_A) TYPE  /ADZ/ENET_PREISE_T
*"     REFERENCE(OFF_B) TYPE  /ADZ/ENET_PREISE_T
*"     REFERENCE(OFF_C) TYPE  /ADZ/ENET_PREISE_T
*"     REFERENCE(ABU_A) TYPE  /ADZ/ENET_PREISE_T
*"     REFERENCE(ABU_B) TYPE  /ADZ/ENET_PREISE_T
*"     REFERENCE(ABU_C) TYPE  /ADZ/ENET_PREISE_T
*"----------------------------------------------------------------------

  DATA: ls_umlagen       TYPE /adz/umlagen,
        lt_umlagen       TYPE TABLE OF /adz/umlagen,
        lv_ab            TYPE dats,
        lv_bis           TYPE dats,
        lv_ende          TYPE c,
  ...
  LOOP AT lt_string INTO lv_string.
    ...
    CLEAR ls_umlagen.
    DO 100 TIMES.
      SELECT SINGLE * FROM /adz/umlagen INTO ls_umlagen WHERE netz_nr = netz_nr AND gueltig_seit =< lv_ab AND gueltig_bis => lv_ab AND preistyp = lv_select_string .
      "Zum Beginn der Rechnung ist keine Zeitscheibe vorhanden.
      IF sy-subrc <> 0.
        "erste Zeitscheibe bestimmen
        SELECT * FROM /adz/umlagen INTO TABLE lt_umlagen WHERE netz_nr = netz_nr AND preistyp = lv_select_string .
