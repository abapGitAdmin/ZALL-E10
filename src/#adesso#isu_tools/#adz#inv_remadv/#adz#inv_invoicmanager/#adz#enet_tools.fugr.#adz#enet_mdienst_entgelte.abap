FUNCTION /ADZ/ENET_MDIENST_ENTGELTE.
*"----------------------------------------------------------------------
...
*"----------------------------------------------------------------------
  ...
  DATA: ls_marktpartner     TYPE /adz/marktpa,
        ls_zadenet_messprei TYPE /adz/messprei,
        ls_messprei         TYPE /adz/messprei,
        ls_preisregelungen  TYPE /adz/messpreg,
        lv_zahelrnr         TYPE i,
        lv_ab               TYPE dats,
        lv_bis              TYPE dats,
        ls_edef_har         TYPE /adz/edef_har,
        lv_ende             TYPE c,
        ls_preise           TYPE LINE OF /adz/enet_preise_t.

  DATA: lt_hardware TYPE TABLE OF /adz/hardware,
        ls_zaehler  TYPE /adz/zaehler,
        ls_hardware TYPE /adz/hardware.

  lv_zahelrnr = zaehlernr.

  SELECT SINGLE * FROM /adz/marktpa
  ...
  DO 100 TIMES.
    ...
    SELECT SINGLE * FROM /adz/messpreg
    ...
    DO 5 TIMES.
      ...
      SELECT SINGLE * FROM /adz/messprei
      ...
      IF sy-subrc = 0.
        ...
        SELECT * FROM /adz/hardware INTO TABLE lt_hardware WHERE messpreis_id = ls_zadenet_messprei-messpreis_id.
        ...
        ELSE.
          SELECT SINGLE * FROM /adz/zaehler INTO ls_zaehler WHERE zaehler_id = lv_zahelrnr.
          IF sy-index = 2.
            SELECT * FROM   /adz/edef_har INTO ls_edef_har
              ...
              IF sy-subrc <> 0.
                SELECT SINGLE * FROM /adz/messprei INTO ls_messprei
    ...
    SELECT * FROM /adz/messprei INTO ls_zadenet_messprei
      WHERE zaehler_id = 0 AND hardware_id <> 0 AND
      preisregelung_id = ls_preisregelungen-preisregelung_id.
      " AND standard = '1'.

      SELECT SINGLE * FROM /adz/edef_har INTO ls_edef_har WHERE hardware_id = ls_zadenet_messprei-hardware_id.
    ...
    SELECT * FROM /adz/messprei INTO ls_zadenet_messprei
      ...
      SELECT SINGLE * FROM /adz/edef_die INTO ls_edef_har WHERE dienstleistung_id = ls_zadenet_messprei-dienstleistung_id.
