FUNCTION /ADZ/ENET_GET_PREIS_ANL_GAS.
*"----------------------------------------------------------------------
...
*"  CHANGING
*"     REFERENCE(ARTIKEL_PREIS) TYPE  /ADZ/ENET_PREIS_ARTIKEL_T
*"       OPTIONAL
*"  EXCEPTIONS
*"      KEIN_NETZ
*"----------------------------------------------------------------------
  TYPES: BEGIN OF t_preise,
           ...
           dienstl TYPE /adz/enet_preise_t,
           gp      TYPE /adz/enet_preise_t,
           ap      TYPE /adz/enet_preise_t,
           lp      TYPE /adz/enet_preise_t,
           farb    TYPE /adz/enet_preise_t,
           flei    TYPE /adz/enet_preise_t,
  ...
  DATA: ls_eanl           TYPE eanl,
        ...
        lv_spebene(2)     TYPE c,
        ls_eanlh          TYPE eanlh,
        nd_vnbg_nr        TYPE  /adz/enet_gas_vnbg_nr,
        nd_teilnetz_nr    TYPE  /adz/enet_gas_teilnetz_nr,
        nd_netzbereich_nr TYPE  /adz/enet_gas_netzber_nr,
        md_vnbg_nr        TYPE  /adz/enet_gas_vnbg_nr,
        md_teilnetz_nr    TYPE  /adz/enet_gas_teilnetz_nr,
        md_netzbereich_nr TYPE  /adz/enet_gas_netzber_nr,
        hd_vnbg_nr        TYPE  /adz/enet_gas_vnbg_nr,
        hd_teilnetz_nr    TYPE  /adz/enet_gas_teilnetz_nr,
        hd_netzbereich_nr TYPE  /adz/enet_gas_netzber_nr,
        ka_id             TYPE  /adz/enet_gas_vnbg_nr,
        lv_rlmslp         TYPE c, "1 = RLM 0 = SLP
        lv_custrlmslp     TYPE string,
        lv_custreport     TYPE string,
        lv_vnbg_nr        TYPE  /adz/enet_gas_vnbg_nr,
        lv_teilnetz_nr    TYPE  /adz/enet_gas_teilnetz_nr,
        lv_netzbereich_nr TYPE  /adz/enet_gas_netzber_nr,
        ls_netzbereich    TYPE /adz/g_bereic.
...
* Implement suitable error handling here
  ...
  CALL FUNCTION '/ADZ/ENET_NETZE_ZU_ADR_GAS'
    EXPORTING
      adrc              = ls_adrc
      ab                = abr_ab
    IMPORTING
...
"  SELECT SINGLE adspebene FROM /adz/ec_spebn INTO lv_spebene WHERE custspebene = ls_eanl-spebene AND sparte = 'GA'.
  CASE lv_spebene.
    WHEN 'NV' or 'ND'.
  ...
  SELECT SINGLE * FROM /adz/g_bereic
    INTO ls_netzbereich
  ...
  CALL FUNCTION '/ADZ/ENET_ZAEHLERNR_GAS'
    EXPORTING
      anlage       = anlage
      abr_preis    = abr_preis
    ...
    CALL FUNCTION '/ADZ/ENET_MDIENST_ENT_GAS'
      EXPORTING
...
CALL FUNCTION '/ADZ/ENET_ABRECHNUNG_GAS'
  EXPORTING
    vnbg_nr           = ls_netzbereich-vnbg_nr
    tarifgebiet       = ls_netzbereich-tarifgebiet
    anlage            = anlage
...
CALL FUNCTION '/ADZ/ENET_NNE_GAS'
  EXPORTING
    vnbg_nr              = ls_netzbereich-vnbg_nr
    tarifgebiet          = ls_netzbereich-tarifgebiet
    anlage               = anlage
...
CALL FUNCTION '/ADZ/ENET_KA_GAS'
 ...
 IMPORTING
  ...
  CALL FUNCTION '/ADZ/ENET_MAPPING_GAS'
    EXPORTING
      i_preise   = ls_preise
    IMPORTING
  ...
  IF display = 'X'.
    ...
    CALL FUNCTION '/ADZ/ENET_DISPLAY_GAS'
      EXPORTING
        i_preise = ls_preise.
