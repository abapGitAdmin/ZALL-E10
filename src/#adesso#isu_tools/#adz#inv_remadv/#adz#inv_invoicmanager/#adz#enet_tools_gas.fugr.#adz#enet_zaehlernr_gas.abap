FUNCTION /ADZ/ENET_ZAEHLERNR_GAS.
*"----------------------------------------------------------------------
*"*"Lokale Schnittstelle:
*"  IMPORTING
*"     REFERENCE(ANLAGE) TYPE  ANLAGE
*"     REFERENCE(ABR_PREIS) TYPE  /ADZ/ENET_PREIS OPTIONAL
*"     REFERENCE(VNBG_NR) TYPE  /ADZ/ENET_GAS_VNBG_NR OPTIONAL
*"     REFERENCE(TARIFGEBIET) TYPE  /ADZ/ENET_GAS_TARIFGEBIET OPTIONAL
...
*"----------------------------------------------------------------------
  ...
  DATA:   ls_isu07_install_struc TYPE isu07_install_struc,
          ...
          lv_zaehl               TYPE /adz/zaehlerd,
          ls_zaehl               TYPE /adz/ec_zaehl,
...
*     Y_INSTAL_STRUC_OLD       =
  ...
  IF zahlernummer IS  INITIAL.
    ...
    SELECT  * FROM /adz/ec_zaehl INTO ls_zaehl WHERE preis = ls_eastl-preiskla AND sparte = 'GA'.
      SELECT SINGLE zaehler_id FROM /adz/g_zaehlr INTO zahlernummer WHERE zaehler_id = ls_zaehl-zaehler AND lieferstelle = lv_lieferstelle AND druckstufe = lv_druckstufe.
      IF sy-subrc = 0.
        EXIT.
      ENDIF.
    ENDSELECT.
    IF zahlernummer IS INITIAL.
      SELECT  * FROM /adz/ec_zaehl INTO ls_zaehl WHERE preis = ls_eastl-preiskla(4) AND sparte = 'GA'.
        SELECT SINGLE zaehler_id FROM /adz/g_zaehlr INTO zahlernummer WHERE zaehler_id = ls_zaehl-zaehler AND lieferstelle = lv_lieferstelle AND druckstufe = lv_druckstufe.
        IF sy-subrc = 0.
          EXIT.
        ENDIF.
    ...
    ENDIF.
    IF zahlernummer IS INITIAL.
      SELECT SINGLE zaehler_id FROM /adz/g_abrpr INTO lv_zaehl WHERE vnbg_nr = vnbg_nr AND tarifgebiet = tarifgebiet AND preis = abr_preis.
      IF sy-subrc = 0.
