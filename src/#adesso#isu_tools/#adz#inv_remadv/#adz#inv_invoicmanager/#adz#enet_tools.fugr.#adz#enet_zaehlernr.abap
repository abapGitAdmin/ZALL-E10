...
*     Y_INSTAL_STRUC_OLD       =
  ...
  SELECT SINGLE * FROM /adz/ec_zaehl INTO ls_zaehl WHERE preis = ls_eastl-preiskla AND sparte = 'ST'.
  IF sy-subrc = 0.
    zahlernummer = ls_zaehl-zaehler.
  ELSE.
    SELECT SINGLE zaehler_id FROM /adz/abrech INTO lv_zaehl WHERE netz_nr = netz_nr AND preis = abr_preis.
    IF sy-subrc = 0.
