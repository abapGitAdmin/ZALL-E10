...
*  SELECT SINGLE spannungsebene FROM /adz/zaehler INTO lv_spebene_enet WHERE zaehler_id = lv_zahelrnr.
  lv_ausgrup = ls_ever-ausgrup.
  SELECT SINGLE kundenart FROM /adz/ec_ausgr INTO lv_kuart WHERE ausgrup = lv_ausgrup.
  IF lv_kuart = 1.
    lv_hh_gw = 'GW_'.
  ELSE.
    lv_hh_gw = 'HH_'.
  ...
  DO 100 TIMES.
    "Netzentgelte ziehen
    SELECT SINGLE * FROM /adz/enet_nne INTO ls_nne WHERE netz_nr = netz_nr AND gueltig_seit <= lv_ab AND gueltig_bis => lv_ab.
    IF sy-subrc = 0.
