FUNCTION /adz/enet_upload.
*"----------------------------------------------------------------------
*"*"Lokale Schnittstelle:
*"  IMPORTING
*"     VALUE(FILES) TYPE  /ADZ/ENET_IMPORT_STRINGTAB
...
*"----------------------------------------------------------------------
  ...
  DATA ut_daten_aus_datei TYPE /adz/enet_import_stringtab.
  DATA lv_dats TYPE LINE OF /adz/enet_import_stringtab.
  DATA lv_string TYPE string.
  DATA lv_string_temp TYPE string.
  ut_daten_aus_datei = files.
  uv_filename = filename.
  TRANSLATE uv_filename TO UPPER CASE.
  SELECT SINGLE tabelle FROM /adz/ec_enet INTO lv_tabelle WHERE datei = uv_filename+21.
  IF sy-subrc = 0.
