...
*"  EXPORTING
*"     REFERENCE(AP) TYPE  /ADZ/ENET_PREISE_T
*"     REFERENCE(LP) TYPE  /ADZ/ENET_PREISE_T
*"     REFERENCE(GP) TYPE  /ADZ/ENET_PREISE_T
*"     REFERENCE(FLEI) TYPE  /ADZ/ENET_PREISE_T
*"     REFERENCE(FARB) TYPE  /ADZ/ENET_PREISE_T
*"----------------------------------------------------------------------
  ...
  DATA:
    ...
    lv_index           TYPE i,
    lv_index2          TYPE i,
    lt_vnaslp          TYPE TABLE OF /adz/g_vnaslp,
    ls_vnaslp          TYPE /adz/g_vnaslp,
    lt_vnlslp          TYPE TABLE OF /adz/g_vnlslp,
    ls_vnlslp          TYPE /adz/g_vnlslp,
    lt_vkkslp          TYPE TABLE OF /adz/g_vkkslp,
    ls_vkkslp          TYPE /adz/g_vkkslp,
    lt_vnalgk          TYPE TABLE OF /adz/g_vnalgk,
    ls_vnalgk          TYPE /adz/g_vnalgk,
    lt_vnllgk          TYPE TABLE OF /adz/g_vnllgk,
    ls_vnllgk          TYPE /adz/g_vnllgk,
    lt_vlklgk          TYPE TABLE OF /adz/g_vlklgk,
    ls_vlklgk          TYPE /adz/g_vlklgk,
    lv_exit            TYPE c,
    lt_vaklgk          TYPE TABLE OF /adz/g_vaklgk,
    ls_vaklgk          TYPE /adz/g_vaklgk,
    lv_operarb         TYPE ettifn-operand,
    lv_operlei         TYPE ettifn-operand,
    lt_ettifn          TYPE TABLE OF ettifn,
    ls_ettifn          TYPE ettifn,
    ls_preis           TYPE LINE OF /adz/enet_preise_t.
  ...
  SELECT SINGLE value FROM /adz/inv_cust INTO lv_operarb WHERE report = 'ANLAGEFAKT' AND field = 'ARBEIT_G'.
  SELECT SINGLE value FROM /adz/inv_cust INTO lv_operlei WHERE report = 'ANLAGEFAKT' AND field = 'LEISTUNG_G'.
  ...
  SELECT SINGLE lieferstelle FROM /adz/g_zaehlr INTO lv_lieferstelle WHERE zaehler_id  = zaehlernr.
    ...
    IF lv_lieferstelle = 'SLP'.
      IF lv_index = 1.
        LOOP AT lt_tinv_inv_line_b INTO ls_tinv_inv_line_b WHERE product_id = '9990001000269' AND date_from = ab AND date_to = bis.
          ...
          SELECT  * FROM /adz/g_vnaslp INTO TABLE lt_vnaslp WHERE vnbg_nr = vnbg_nr AND tarifgebiet = tarifgebiet AND gueltig_seit < ls_tinv_inv_line_b-date_from AND id = '01'.
          ...
          SELECT  * FROM /adz/g_vkkslp INTO TABLE lt_vkkslp WHERE vnbg_nr = vnbg_nr AND tarifgebiet = tarifgebiet AND gueltig_seit < ls_tinv_inv_line_b-date_from AND id = '01'.
          SORT lt_vnaslp BY gueltig_seit DESCENDING.
        ...
        LOOP AT lt_tinv_inv_line_b INTO ls_tinv_inv_line_b WHERE product_id = '9990001000053' AND date_from = ab AND date_to = bis.
          ...
          SELECT  * FROM /adz/g_vnlslp INTO TABLE lt_vnlslp WHERE vnbg_nr = vnbg_nr AND tarifgebiet = tarifgebiet AND gueltig_seit < ls_tinv_inv_line_b-date_from AND id = '01'.
    ...
    ELSEIF lv_lieferstelle = 'LGK'.
      ...
      LOOP AT lt_tinv_inv_line_b INTO ls_tinv_inv_line_b WHERE product_id = '9990001000269' AND date_from = ab AND date_to = bis..
        ...
        IF lv_arbeit IS NOT INITIAL.
          SELECT  * FROM /adz/g_vnalgk INTO TABLE lt_vnalgk WHERE vnbg_nr = vnbg_nr AND tarifgebiet = tarifgebiet AND gueltig_seit < ls_tinv_inv_line_b-date_from AND id = '01'.
...
*                  APPEND ls_preis TO ap.
          ...
          SELECT  * FROM /adz/g_vaklgk INTO TABLE lt_vaklgk WHERE vnbg_nr = vnbg_nr AND tarifgebiet = tarifgebiet AND gueltig_seit < ls_tinv_inv_line_b-date_from AND id = '01'.
...
*    ENDIF.
      ...
      LOOP AT lt_tinv_inv_line_b INTO ls_tinv_inv_line_b WHERE product_id = '9990001000053' AND date_from = ab AND date_to = bis..
        ...
        IF lv_leistung IS NOT INITIAL.
          SELECT  * FROM /adz/g_vnllgk INTO TABLE lt_vnllgk WHERE vnbg_nr = vnbg_nr AND tarifgebiet = tarifgebiet AND gueltig_seit < ls_tinv_inv_line_b-date_from AND id = '01'.
          ...
          SELECT  * FROM /adz/g_vlklgk INTO TABLE lt_vlklgk WHERE vnbg_nr = vnbg_nr AND tarifgebiet = tarifgebiet AND gueltig_seit < ls_tinv_inv_line_b-date_from AND id = '01'.
