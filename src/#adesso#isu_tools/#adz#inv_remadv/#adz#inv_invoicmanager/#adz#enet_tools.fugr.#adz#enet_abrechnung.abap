...
*"----------------------------------------------------------------------
  ...
  DATA: ls_abrechnung       TYPE /adz/abrech,
        ls_abrechnung2      TYPE /adz/abrech,
        lv_ab               TYPE d,
        lv_bis              TYPE d,
        lv_zaehler_char(10) TYPE c,
        ls_preise           TYPE /adz/enet_preis_dats,
        lt_abrechnung       TYPE TABLE OF /adz/abrech.

  lv_zaehler_char = zaehlerid.
  SHIFT lv_zaehler_char LEFT DELETING LEADING ' '.
  SELECT * FROM /adz/abrech INTO TABLE lt_abrechnung WHERE netz_nr = netz_nr AND zaehler_id = lv_zaehler_char AND STANDARD = '1' AND Id = '1 '.
