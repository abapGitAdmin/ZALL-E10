...
*"  EXPORTING
*"     REFERENCE(KAT_A) TYPE  /ADZ/ENET_PREISE_T
*"     REFERENCE(KAT_B) TYPE  /ADZ/ENET_PREISE_T
*"     REFERENCE(KAT_C) TYPE  /ADZ/ENET_PREISE_T
*"----------------------------------------------------------------------

  DATA: ls_kwk    TYPE /adz/enet_kwk,
        lv_ab     TYPE dats,
        lv_bis    TYPE dats,
        lv_ende   TYPE c,
  ...
  lv_ab = ab.
  DO 100 TIMES.
    SELECT SINGLE * FROM /adz/enet_kwk INTO ls_kwk WHERE netz_nr = netz_nr AND gueltig_seit =< lv_ab AND gueltig_bis => lv_ab.
