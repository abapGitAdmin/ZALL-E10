FUNCTION /ADZ/INV_MANAGER_PRICE.
*"----------------------------------------------------------------------
...
* MessageID
  ...
  DATA:
    lt_enet_preis TYPE /adz/enet_preis_artikel_t,
    lt_inv_preis  TYPE /adz/enet_preis_artikel_t,
    ls_enet_preis TYPE /adz/preis_artikel_s.
...
*           X_CREATE_MSG_OBJECT       = 'X'
      ...
      DATA ls_preise_sim TYPE /adz/preis_artikel_s.
      DATA lt_preise_sim TYPE TABLE OF /adz/preis_artikel_s.
      DATA lv_richtiger_preis TYPE c.
      DATA lv_falscher_preis TYPE /adz/preis_artikel_s-preis.
      ...
      LOOP AT lt_inv_line_i INTO ls_inv_line_i WHERE price IS NOT INITIAL.
        ...
        SELECT  belzart FROM /adz/art_cust INTO lv_belzart WHERE art_nr = ls_inv_line_i-product_id.
          IF sy-subrc = 0.
      ...
      LOOP AT lt_inv_line_i INTO ls_inv_line_i WHERE price IS NOT INITIAL.
        ...
        SELECT SINGLE adsparte FROM /adz/ec_spart INTO lv_sparte WHERE sparte = ls_ever-sparte.
        SELECT SINGLE * FROM /adz/art_cust INTO ls_art_cust WHERE art_nr = ls_inv_line_i-product_id AND sparte = lv_sparte.
        IF sy-subrc = 0.
...
FORM preis_pruefung_enet USING ut_preise_inv TYPE /adz/enet_preis_artikel_t  ut_preise_enet TYPE /adz/enet_preis_artikel_t uv_sparte TYPE /adz/sparte ab bis.
  DATA:  ls_enet_preis      TYPE /adz/preis_artikel_s,
         lv_found           TYPE c,
         ls_inv_preis       TYPE /adz/preis_artikel_s,
         lv_enet_preis      TYPE /adz/enet_preis,
         ls_art_cust        TYPE /adz/art_cust,
...
*  ENDIF.
  ...
  LOOP AT ut_preise_inv INTO ls_inv_preis WHERE ab >= ab AND bis <= bis .
    ...
    SELECT SINGLE * FROM /adz/art_cust INTO ls_art_cust WHERE art_nr = ls_inv_preis-artikelnr AND sparte = uv_sparte.
    IF sy-subrc = 0.
