...
MODULE user_command_0100 INPUT.
  ...
  DATA ls_invtext         TYPE /adz/invtext.
  IF gv_ok_sim = 'SIM_START'.
    LOOP AT gt_ausgabe_sim REFERENCE INTO DATA(lr_ausgabe_sim).
      ...
      SELECT SINGLE msc_start msc_end FROM /adz/inv_msc INTO (lr_ausgabe_sim->msc_start, lr_ausgabe_sim->msc_end) WHERE int_inv_no = ls_proc_data-inv_head-int_inv_no.
...
FORM select_data CHANGING ct_ausgabe_sim TYPE tt_ausgabe_sim.
  ...
  DATA ls_inv_case TYPE /adz/inv_case.
  ...
  LOOP AT gt_inv_doc_no INTO DATA(ls_inv_doc_no).
    ...
    SELECT SINGLE * FROM /adz/inv_msc INTO ls_inv_msc WHERE int_inv_no = ls_inv_doc_no.
    SELECT SINGLE * FROM /adz/inv_case INTO ls_inv_case WHERE int_inv_no = ls_inv_doc_no.
...
FORM sim_verbrauch CHANGING  u_inv_process_data TYPE inv_process_data  cv_msc.
  ...
  DATA lv_fuba_to_call TYPE string.
  SELECT SINGLE value FROM /adz/inv_cust INTO lv_fuba_to_call   WHERE report = 'GLOBAL' AND field = 'FUBA_SIM_VERBRAUCH'.
  IF sy-subrc NE 0.
    lv_fuba_to_call = 'Z_ADESSOINV_MANAGER_QUANTITY_N'.
  ENDIF.
  CALL FUNCTION lv_fuba_to_call
    EXPORTING
