FUNCTION /adz/inv_manager_quantity .
*"--------------------------------------------------------------------
...
* Datenermittlung zur Lastgangmessung
  ...
  DATA: ls_art_cust TYPE /adz/art_cust,
...
* --- process relevant lines in invoice for quantity-check
          LOOP AT <y_process>-inv_line_b ASSIGNING <line_b>
            ...
            IF 1 = 0. MESSAGE i002(/adz/inv_manager). ENDIF.
...
*         sum of quantity invoiced
          LOOP AT t_quant.
            ...
            IF 1 = 0. MESSAGE i004(/adz/inv_manager). ENDIF.
          ENDLOOP.
...
*         e.g. dateto, datefrom not filled
              ...
              IF 1 = 2. MESSAGE w005(/adz/inv_manager). ENDIF.
              EXIT.
            ELSE.
...
*       No installation with correct service type
            ...
            EXIT.
            IF 1 = 2. MESSAGE w006(/adz/inv_manager). ENDIF.
          ENDIF.
...
*          IF ls_eanl-sparte = '10'.
          ...
          IF ls_eanlh-aklasse = co_sonder.
            is_rlm = 'X'.
          ENDIF.
          DATA ls_tarif TYPE /adz/ec_tarif.
          SELECT SINGLE * FROM /adz/ec_tarif INTO ls_tarif WHERE tarif = ls_eanlh-tariftyp.
          IF sy-subrc = 0.
          ...
          ENDIF.
          DATA lv_cust TYPE c.
          SELECT SINGLE adsparte FROM /adz/ec_spart INTO lv_adsparte WHERE sparte = ls_eanl-sparte.
          ...
          IF lv_cust <> 'X'.
            ...
            DATA ls_pausch TYPE /adz/ec_pausc.
            DATA lt_ettifn TYPE TABLE OF ettifn.
*DATA ls_ettifn TYPE ettifn.
            SELECT * FROM /adz/ec_pausc INTO ls_pausch WHERE tarif = ls_eanlh-tariftyp.
...
*          ENDIF.
            DATA lv_abwv TYPE i.
            DATA lv_abwn TYPE i.
            DATA lv_value TYPE /adz/inv_cust-value.
            SELECT SINGLE value FROM /adz/inv_cust INTO lv_value  WHERE report = 'INV_MANAGER_QUANTITY' AND field = 'ABL_DATEV'.
            IF sy-subrc = 0.
              lv_abwv = lv_value.
              <doc>-invperiod_start = <doc>-invperiod_start - lv_abwv.
            ENDIF.
            SELECT SINGLE value FROM /adz/inv_cust INTO lv_value  WHERE report = 'INV_MANAGER_QUANTITY' AND field = 'ABL_DATEN'.
            IF sy-subrc = 0.
              lv_abwn = lv_value.
...
* Datenermittlung zur Lastgangmessung
              ...
              LOOP AT t_last INTO s_last.
                ...
                SELECT SINGLE adsparte FROM /adz/ec_spart INTO lv_adsparte WHERE sparte = s_eprofhead-sparte.
                IF sy-subrc <> 0.
                  lv_adsparte = 'ST'.
                ENDIF.
...
*                         XY_INST_FOR_BILLING       =
                      ...
                      ELSE.
                        ...
                        IF 1 = 2. MESSAGE i009(/adz/inv_manager). ENDIF.
                      ENDIF.
                    ELSE.
                      CALL FUNCTION 'ISU_CONSUMPTION_DETERMINE'
                        EXPORTING
...
*          ENDIF.                                                "EVNUSS 05.04.2012
              ...
              ELSE.
                ...
                IF 1 = 2. MESSAGE i010(/adz/inv_manager). ENDIF.
              ENDIF.
...
**      <-- Akaslan 23.01.2015 SR-1501341
          DO 2 TIMES.
            ...
            SELECT SINGLE * FROM /adz/art_cust INTO ls_art_cust WHERE art_nr = t_quant-product_id AND sparte = lv_adsparte.
            IF sy-subrc = 0.
            ...
            IF abs( l_diff ) GT lv_abw_abs AND abs( l_diff ) / t_quant-quantity * 100 > lv_abw_proz.
              ...
              ELSE.
                msg_to_inv_return space co_msg_error co_z_msgid '033'
                                 c_quantity c_consumpt t_quant-unit space .
              ENDIF.
              IF 1 = 2. MESSAGE e011(/adz/inv_manager). ENDIF.
            ELSE.
              IF sy-index = 1.
              ...
              IF 1 = 2. MESSAGE i012(/adz/inv_manager). ENDIF.
            ENDIF.
          ENDDO.
        ENDIF.
      ENDIF.
...
* success!
  IF f_rc IS INITIAL.
    ...
    IF 1 = 2.
      MESSAGE s013(/adz/inv_manager) WITH val1.
    ENDIF.
  ENDIF.
...
FORM cust_pruef USING u_sparte u_rlm u_start u_end u_anlage CHANGING c_cust c_verbr.
  ...
  IF u_sparte = 'ST' AND u_rlm = 'X'.
    SELECT SINGLE value FROM /adz/inv_cust INTO lv_custform WHERE report = 'GLOBAL' AND field = 'VP_RLM_STROM_FORM'.
    IF lv_custform IS NOT INITIAL AND lv_custreport IS NOT INITIAL.

    ENDIF.
  ELSEIF u_sparte = 'ST' AND u_rlm = ''.
    SELECT SINGLE value FROM /adz/inv_cust INTO lv_custform WHERE report = 'GLOBAL' AND field = 'VP_SLP_STROM_FORM'.
    IF lv_custform IS NOT INITIAL AND lv_custreport IS NOT INITIAL.

    ENDIF.
  ELSEIF u_sparte = 'GAS' AND u_rlm = ''.
    SELECT SINGLE value FROM /adz/inv_cust INTO lv_custform WHERE report = 'GLOBAL' AND field = 'VP_SLP_GAS_FORM'.
    IF lv_custform IS NOT INITIAL AND lv_custreport IS NOT INITIAL.
      CALL FUNCTION '/ADZ/CONSUMPT_SLP_GAS_CUST'
        EXPORTING
          anlage       = u_anlage
          datbis       = u_end
          datab        = u_start
  ...
  ELSEIF u_sparte = 'GAS' AND u_rlm = 'X'.
    SELECT SINGLE value FROM /adz/inv_cust INTO lv_custform WHERE report = 'GLOBAL' AND field = 'VP_RLM_GAS_FORM'.
    IF lv_custform IS NOT INITIAL AND lv_custreport IS NOT INITIAL.
