FUNCTION /ADZ/REMADV_BEMERKUNG_ANL.
*"----------------------------------------------------------------------
...
*"----------------------------------------------------------------------
  ...
  DATA ls_remtext TYPE /adz/remtext.
  CLEAR: ok, bemerkung, gt_bemerkung, gs_bemerkung.

  lt_int_doc_no = int_inv_doc_no.
  SELECT * FROM /adz/remtext INTO CORRESPONDING FIELDS OF TABLE gt_bemerkung FOR ALL ENTRIES IN lt_int_doc_no WHERE int_inv_doc_nr  = lt_int_doc_no-int_doc_no.
  IF sy-subrc <> 0.
  ...
  IF ok = 'SAV'.
    LOOP AT int_inv_doc_no INTO ls_int_doc_no.
      ...
      INSERT INTO /adz/remtext VALUES ls_remtext.
    ENDLOOP.
  ENDIF.
