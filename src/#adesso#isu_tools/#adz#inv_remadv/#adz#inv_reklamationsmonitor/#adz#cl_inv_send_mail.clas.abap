CLASS /adz/cl_inv_send_mail DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
...
CLASS /adz/cl_inv_send_mail IMPLEMENTATION.
  METHOD constructor.
...
* METH_INFO "Dialog für den Mailversand öffnen und vordefinierte Werte übergeben"
  METHOD send_mail.
    ...
    DATA: lv_reply  TYPE /adz/fi_remad-negrem_value,
          lv_sender TYPE /adz/fi_remad-negrem_value,
...
*    Antwortadresse
    /adz/cl_inv_customizing_data=>get_config_value( EXPORTING iv_option = 'MAIL'
                                                iv_category = 'REPLY'
                                                iv_field = 'ADDRESS'
                                                iv_id = '1'
...
*    EXPORTING #1 = <file-name-on-c:drive>.
  ...
  METHOD send_lotus_mail.
    ...
    DATA: wa_lotus TYPE /adz/fi_lotus,
          datei    TYPE char255.
...
* Customizingtabelle muss gepflegt sein
    SELECT SINGLE * FROM /adz/fi_lotus INTO wa_lotus
       WHERE bname = sy-uname.

    IF sy-subrc NE 0.
      MESSAGE i000(e4) WITH 'Für User'
                            sy-uname
                            'ist Tabelle /EVUIT/FI_LOTUS'
                            'nicht gepflegt' DISPLAY LIKE 'E'.
      EXIT.
    ENDIF.
...
*   Den Benutzernamen im Klartext zurückgeben
  ...
  METHOD get_baustein.
    ...
    DATA: it_bausteine TYPE TABLE OF /adz/fi_remad.
    DATA: lv_text TYPE string.

    /adz/cl_inv_customizing_data=>get_config( EXPORTING iv_option = 'TEXT'
    ...
    FIELD-SYMBOLS: <fs_baustein> TYPE /adz/fi_remad.
    lv_readtext = abap_false.

    IF lines( it_bausteine ) > 1.

      DATA: it_selection TYPE TABLE OF /adz/fi_remad.
      /adz/cl_inv_customizing_data=>get_config( EXPORTING iv_option = 'TEXT'
                                                  iv_category = iv_text_typ
                                                  iv_field = 'BEZEICHNUNG'
                                        RECEIVING rv_t_values = it_selection ).

      DATA: tmpbaustein TYPE /adz/fi_remad.
    ...
    CLEAR it_selection.
    /adz/cl_inv_customizing_data=>get_config( EXPORTING iv_option   = <fs_baustein>-negrem_option
    ...
    DATA: wa_cust     TYPE /adz/fi_remad,
  ...
  METHOD baustein_selection.
    ...
    DATA: lv_t_values TYPE TABLE OF /adz/fi_neg_remadv_val .
    DATA: lv_text TYPE string.
    DATA: wa_cust TYPE /adz/fi_remad.
    ...
    CALL FUNCTION 'POPUP_WITH_TABLE_DISPLAY'
      ...
      EXCEPTIONS
        ...
        OTHERS       = 2.
    IF sy-subrc <> 0.
      MESSAGE i000(e4) WITH 'Fehler beim Aufruf der Textbausteinauswahl.' DISPLAY LIKE 'E'.
      EXIT.
    ENDIF.
