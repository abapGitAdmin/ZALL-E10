FUNCTION /adesso/enet_get_prices_anlage.
*"----------------------------------------------------------------------
...
*"  CHANGING
*"     VALUE(ARTIKEL_PREIS) TYPE  /ADESSO/ENET_PREIS_ARTIKEL_T OPTIONAL
*"  EXCEPTIONS
*"      KEIN_NETZ
*"----------------------------------------------------------------------
  ...
  TYPES: BEGIN OF t_preise,
           ...
           blind   TYPE /adesso/enet_preise_t,
           ...
           skap    TYPE /adesso/enet_preise_t,
           skapp   TYPE /adesso/enet_preise_t,
           sku_a   TYPE /adesso/enet_preise_t,
           sku_b   TYPE /adesso/enet_preise_t,
           sku_c   TYPE /adesso/enet_preise_t,
           off_a   TYPE /adesso/enet_preise_t,
           off_b   TYPE /adesso/enet_preise_t,
           off_c   TYPE /adesso/enet_preise_t,
           abu_a   TYPE /adesso/enet_preise_t,
           abu_b   TYPE /adesso/enet_preise_t,
           abu_c   TYPE /adesso/enet_preise_t,
         END OF t_preise.
  ...
  DATA:
    ...
    ls_enet_zaehler TYPE /adesso/zaehler,
    ls_eanl         TYPE eanl,
    ls_eanlh        TYPE eanlh,
    lt_adrc         TYPE ieadrc,
    ls_adrc         TYPE adrc,
    ...
    lv_spebene      TYPE /adesso/spebene,
    ls_enet_netze   TYPE /adesso/netze.
...
* Implement suitable error handling here
  ...
  CALL FUNCTION '/ADESSO/ENET_NETZE_ZU_ADRESSE'
    EXPORTING
      adrc      = ls_adrc
      ab        = abr_ab
    IMPORTING
  ...
  SELECT SINGLE adspebene FROM /adesso/ec_spebn INTO lv_spebene WHERE custspebene = ls_eanl-spebene AND sparte = 'ST'.
  CASE lv_spebene.
    WHEN 1.
  ...
  SELECT SINGLE * FROM /adesso/netze
    INTO ls_enet_netze
  ...
  CALL FUNCTION '/ADESSO/ENET_ZAEHLERNR'
    EXPORTING
      anlage       = anlage
      netz_nr      = ls_netz_nr
      abr_preis    = abr_preis
  ...
  CALL FUNCTION '/ADESSO/ENET_MDIENST_ENTGELTE'
    EXPORTING
...
* Implement suitable error handling here
  ...
  CALL FUNCTION '/ADESSO/ENET_BLINDSTROM'
    EXPORTING
      netz_nr        = ls_netz_nr
      ab             = abr_ab
      bis            = abr_bis
  ...
  CALL FUNCTION '/ADESSO/ENET_UMLAGE'
    EXPORTING
      netz_nr = ls_netz_nr
      ab      = abr_ab
      bis     = abr_bis
  ...
  CALL FUNCTION '/ADESSO/ENET_ABRECHNUNG'
    EXPORTING
      netz_nr   = ls_netz_nr
      anlage    = anlage
  ...
  CALL FUNCTION '/ADESSO/ENET_MAPPING'
    EXPORTING
      i_preise   = ls_preise
    IMPORTING
