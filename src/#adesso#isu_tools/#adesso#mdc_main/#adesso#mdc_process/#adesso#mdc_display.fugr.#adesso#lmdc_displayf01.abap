*----------------------------------------------------------------------*
***INCLUDE /ADESSO/LMDC_DISPLAYF01.
*----------------------------------------------------------------------*
*&---------------------------------------------------------------------*
...
***** Daten neu lesen *****************************************************************************
  TRY.
      ...
      CALL METHOD /adesso/cl_mdc_check_method=>update_step_data_from_system
        EXPORTING
          is_process_step_key = gs_proc_step_key
        CHANGING
          cr_data             = lr_dref
          cr_data_log         = lr_dref_log.
      CALL METHOD /adesso/cl_mdc_check_method=>update_compare_result_select
        EXPORTING
          is_process_step_key = gs_proc_step_key
        CHANGING
          cr_data             = lr_dref
...
***** interne Tabelle/ALV mit DATEN füllen ********************************************************
  ...
  LOOP AT gs_proc_step_data-mtd_code_result ASSIGNING <fs_mtd_code_result>.
    ...
    TRY.
        ls_mdc_in = /adesso/cl_mdc_customizing=>get_inbound_config_for_edifact( iv_edifact_structur = lv_edifact_structur iv_assoc_servprov = gs_proc_step_data-assoc_servprov ).
    ...
    ELSE.
      ...
      CALL FUNCTION 'ISU_DOMVALUE_TEXT_GET'
        EXPORTING
          x_name  = '/ADESSO/MDC_AUTO_CHANGE_RSP'
          x_value = lv_dom_value
        IMPORTING
          y_text  = <fs_seltab>-auto_change.
    ENDIF.
