class /ADESSO/CL_MDC_IM_PRO_PARTNER definition
  public
  create public .
...
CLASS /ADESSO/CL_MDC_IM_PRO_PARTNER IMPLEMENTATION.
  ...
  METHOD /adesso/if_mdc_pro_chg~change_auto.
    DATA: lr_utility          TYPE REF TO /idxgc/cl_utility_generic,
          ...
          lv_flag_same_name   TYPE flag,
          lv_flag_same_adr    TYPE flag.
      ...
      LOOP AT lt_pdoc_data ASSIGNING <fs_pdoc_data> WHERE switchnum <> is_proc_step_data-proc_ref.
        LOOP AT <fs_pdoc_data>-msg_data ASSIGNING <fs_msg_data>.
          LOOP AT <fs_msg_data>-name_address TRANSPORTING NO FIELDS
            WHERE party_func_qual = /adesso/if_mdc_co=>gc_edifact_nad_z05.
...
***** Keine korrespondierende Nachricht gefunden, dann neuen GP anlegen ***************************
    IF lines( lt_proc_ref ) = 0.
*---- Kein GP zur Ablesekarte vorhanden: Neuen GP mit Daten vom aktuellen GP anlegen --------------
      IF ls_mrcontact IS INITIAL.
        ...
        TRY.
            lv_contact_bp = /adesso/cl_mdc_utility=>create_partner_from_pdoc_data( is_name_address = ls_name_address ).
...
*---- GP zur Ablesekarte vorhanden: Prüfen ob Daten identisch zu neuem GP -------------------------
      ELSE.
*.... Daten lesen .................................................................................
        TRY.
            lr_utility = /idxgc/cl_utility_generic=>get_instance( ).
            ...
            CALL METHOD lr_utility->get_partner_name_addr_data
              EXPORTING
                iv_bu_partner     = ls_mrcontact-contact_bp
                iv_key_date       = is_proc_step_data-proc_date
              IMPORTING
                es_name_addr_data = ls_name_address.
        ...
        TRY.
            ASSIGN is_proc_step_data-name_address[ party_func_qual = /idxgc/if_constants_ide=>gc_nad_qual_ud ] TO <fs_name_address_ud>.
          CATCH cx_sy_itab_line_not_found.
        ENDTRY.
        TRY.
            ASSIGN is_proc_step_data-name_address[ party_func_qual = /idxgc/if_constants_ide=>gc_nad_qual_z04 ] TO <fs_name_address_z04>.
          CATCH cx_sy_itab_line_not_found.
        ENDTRY.

*.... Daten vergleichen ...........................................................................
        IF <fs_name_address_ud> IS ASSIGNED                                         AND
           ls_name_address-name_format_code = <fs_name_address_ud>-name_format_code AND
           ls_name_address-fam_comp_name1   = <fs_name_address_ud>-fam_comp_name1   AND
           ls_name_address-fam_comp_name2   = <fs_name_address_ud>-fam_comp_name2   AND
           ls_name_address-first_name       = <fs_name_address_ud>-first_name       AND
           ls_name_address-name_add1        = <fs_name_address_ud>-name_add1        AND
           ls_name_address-name_add2        = <fs_name_address_ud>-name_add2        AND
           ls_name_address-ad_title_ext     = <fs_name_address_ud>-ad_title_ext.
          lv_flag_same_name = abap_true.
        ENDIF.

        IF <fs_name_address_z04> IS ASSIGNED                                        AND
           ls_name_address-streetname       = <fs_name_address_z04>-streetname      AND
           ls_name_address-houseid_compl    = <fs_name_address_z04>-houseid_compl   AND
           ls_name_address-poboxid          = <fs_name_address_z04>-poboxid         AND
           ls_name_address-postalcode       = <fs_name_address_z04>-postalcode      AND
           ls_name_address-district         = <fs_name_address_z04>-district        AND
           ls_name_address-cityname         = <fs_name_address_z04>-cityname        AND
           ls_name_address-countrycode      = <fs_name_address_z04>-countrycode.
          lv_flag_same_adr = abap_true.
        ENDIF.

*.... Ggf. GP zur Ablesekarte abgrenzen ...........................................................
        IF lv_flag_same_name = abap_true AND lv_flag_same_adr = abap_true.
...
***** Daten für automatische Verbuchung ermitteln *************************************************
...
***** Automatische Verbuchung durchführen *********************************************************
  ...
  METHOD /adesso/if_mdc_pro_chg~change_manual.
    ...
    CALL FUNCTION '/ADESSO/MDC_CHANGE_PARTNER' STARTING NEW TASK 'change_partner_1'
      EXPORTING
...
***** Geschäftspartnername ************************************************************************
    READ TABLE is_proc_step_data-name_address ASSIGNING <fs_name_address> WITH KEY party_func_qual = /idxgc/if_constants_ide=>gc_nad_qual_ud.
    IF sy-subrc = 0.
      IF ( <fs_name_address>-name_format_code = /idxgc/if_constants_ide=>gc_name_format_code_person  AND
        ...
        LOOP AT is_proc_step_data-mtd_code_result ASSIGNING <fs_mtd_code_result>
          WHERE compname = /adesso/if_mdc_co=>gc_compname_name_address AND ref_id = /idxgc/if_constants_ide=>gc_nad_qual_ud.
...
***** Geschäftspartneradresse **************************************************
    READ TABLE is_proc_step_data-name_address ASSIGNING <fs_name_address> WITH KEY party_func_qual = /idxgc/if_constants_ide=>gc_nad_qual_z04.
    IF sy-subrc = 0.
      lv_valdt = is_proc_step_data-proc_date.
      ...
      IF sy-subrc = 0.
        LOOP AT is_proc_step_data-mtd_code_result ASSIGNING <fs_mtd_code_result>
          WHERE compname = /adesso/if_mdc_co=>gc_compname_name_address AND ref_id = /idxgc/if_constants_ide=>gc_nad_qual_z04.
          CASE <fs_mtd_code_result>-fieldname.
