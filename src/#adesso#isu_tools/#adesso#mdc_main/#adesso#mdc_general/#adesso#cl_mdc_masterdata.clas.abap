class /ADESSO/CL_MDC_MASTERDATA definition
  public
  final
  create public .
...
CLASS /ADESSO/CL_MDC_MASTERDATA IMPLEMENTATION.
  ...
  METHOD get_anlage.
    ...
    CALL FUNCTION 'ISU_DB_EUIINSTLN_SELECT'
      ...
      EXCEPTIONS
        ...
        OTHERS        = 4.
    IF sy-subrc <> 0.
      MESSAGE i002(/adesso/mdc_general) WITH 'EUIINSTLN' iv_ext_ui iv_keydate INTO gv_msgtext.
  ...
  METHOD get_ever.
    ...
    IF iv_int_ui IS INITIAL AND iv_ext_ui IS NOT INITIAL.
      lv_int_ui = /adesso/cl_mdc_masterdata=>get_int_ui( iv_ext_ui  = iv_ext_ui iv_keydate = iv_keydate ).
    ELSE.
      lv_int_ui = iv_int_ui.
    ENDIF.

    IF iv_anlage IS INITIAL.
      lv_anlage = /adesso/cl_mdc_masterdata=>get_anlage( iv_int_ui  = lv_int_ui iv_keydate = iv_keydate ).
    ENDIF.
  ...
  METHOD get_ext_ui.
    ...
    IF sy-subrc <> 0.
      MESSAGE i002(/adesso/mdc_general) WITH 'EUITRANS' iv_int_ui iv_keydate INTO gv_msgtext.
      /idxgc/cx_general=>raise_exception_from_msg( ).
    ELSE.
    ...
    IF iv_ext_ui IS NOT INITIAL.
      CALL FUNCTION 'ISU_DB_EUITRANS_EXT_SINGLE'
        ...
        EXCEPTIONS
          ...
          OTHERS       = 3.
      IF sy-subrc <> 0.
        MESSAGE i002(/adesso/mdc_general) WITH 'EUITRANS' iv_ext_ui iv_keydate INTO gv_msgtext.
        /idxgc/cx_general=>raise_exception_from_msg( ).
      ELSE.
  ...
  METHOD get_servprov.
    ...
    IF rs_servprov IS INITIAL.
      MESSAGE i001(/adesso/mdc_general) WITH 'ESERVPROV' iv_serviceid INTO gv_msgtext.
      /idxgc/cx_general=>raise_exception_from_msg( ).
    ENDIF.
  ENDMETHOD.
  ...
  METHOD get_sparte.
    ...
    IF iv_int_ui IS NOT INITIAL.
      ...
      IF ls_v_eanl-sparte IS INITIAL.
        MESSAGE i101(/adesso/mdc_general) WITH iv_int_ui INTO gv_msgtext.
        /idxgc/cx_general=>raise_exception_from_msg( ).
      ELSE.
    ...
    ELSEIF iv_serviceid IS NOT INITIAL.
      ...
      IF ls_tecde-division IS INITIAL.
        MESSAGE i100(/adesso/mdc_general) WITH iv_serviceid INTO gv_msgtext.
        /idxgc/cx_general=>raise_exception_from_msg( ).
      ELSE.
    ...
    ELSEIF iv_anlage IS NOT INITIAL.
      ...
      IF rv_sparte IS INITIAL.
        MESSAGE i102(/adesso/mdc_general) WITH iv_anlage INTO gv_msgtext.
        /idxgc/cx_general=>raise_exception_from_msg( ).
      ENDIF.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD get_v_eanl.
    ...
    IF sy-subrc <> 0.
      MESSAGE i002(/adesso/mdc_general) WITH 'V_EANL' iv_anlage INTO gv_msgtext.
      /idxgc/cx_general=>raise_exception_from_msg( ).
    ENDIF.
  ENDMETHOD.
ENDCLASS.
