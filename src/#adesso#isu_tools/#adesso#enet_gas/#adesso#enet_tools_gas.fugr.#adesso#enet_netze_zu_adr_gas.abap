FUNCTION /ADESSO/ENET_NETZE_ZU_ADR_GAS.
*"--------------------------------------------------------------------
...
*"  EXPORTING
*"     REFERENCE(ND_VNBG_NR) TYPE  /ADESSO/ENET_GAS_VNBG_NR
*"     REFERENCE(ND_TEILNETZ_NR) TYPE  /ADESSO/ENET_GAS_TEILNETZ_NR
*"     REFERENCE(ND_NETZBEREICH_NR) TYPE  /ADESSO/ENET_GAS_NETZBER_NR
*"     REFERENCE(MD_VNBG_NR) TYPE  /ADESSO/ENET_GAS_VNBG_NR
*"     REFERENCE(MD_TEILNETZ_NR) TYPE  /ADESSO/ENET_GAS_TEILNETZ_NR
*"     REFERENCE(MD_NETZBEREICH_NR) TYPE  /ADESSO/ENET_GAS_NETZBER_NR
*"     REFERENCE(HD_VNBG_NR) TYPE  /ADESSO/ENET_GAS_VNBG_NR
*"     REFERENCE(HD_TEILNETZ_NR) TYPE  /ADESSO/ENET_GAS_TEILNETZ_NR
*"     REFERENCE(HD_NETZBEREICH_NR) TYPE  /ADESSO/ENET_GAS_NETZBER_NR
*"     REFERENCE(KA_ID) TYPE  /ADESSO/ENET_GAS_VNBG_NR
*"  EXCEPTIONS
*"      KEIN_NETZ
*"--------------------------------------------------------------------
  DATA: ls_adrc            TYPE adrc,
        ls_zadenet_plz_hnr TYPE /adesso/g_n_haus,
        ls_zadenet_plz_str TYPE /adesso/g_n_str,
        ls_zadenet_plz_ot  TYPE /adesso/g_n_orte,
        ls_zadenet_plz     TYPE /adesso/g_plz_nb.



  ls_adrc = adrc.
  "Netz auf hausnummernebene
  SELECT SINGLE * FROM /adesso/g_n_haus
  ...
  SELECT SINGLE * FROM /adesso/g_n_str
  ...
  SELECT SINGLE * FROM /adesso/g_n_orte
  ...
  IF sy-subrc = 0.
    ...
    ENDIF.
    SELECT SINGLE * FROM /adesso/g_plz_nb
      INTO ls_zadenet_plz
