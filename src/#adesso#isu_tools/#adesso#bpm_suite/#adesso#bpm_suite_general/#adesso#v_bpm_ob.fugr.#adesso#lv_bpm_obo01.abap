...
MODULE set_dp_class OUTPUT.
  ...
  TRY.
      ls_gen_cust = /adesso/cl_bpm_utility=>det_gen_cust( iv_bparea = /adesso/v_bpm_ob-bparea ).
    CATCH /adesso/cx_bpm_utility.
  ENDTRY.

  CALL FUNCTION '/ADESSO/FM_SET_DP_CLASS_NAME'
    EXPORTING
      iv_dp_class = ls_gen_cust-ad_bpm_dp_cl_obj.
...
MODULE chk_dp_method INPUT.
  ...
  TRY.
      ls_gen_cust = /adesso/cl_bpm_utility=>det_gen_cust( iv_bparea = /adesso/v_bpm_ob-bparea ).
    CATCH /adesso/cx_bpm_utility.
      MESSAGE e003(/adesso/bpm_utility) WITH ls_gen_cust-ad_bpm_dp_cl_obj.
  ENDTRY.

  TRY.
      CREATE OBJECT lr_dp_class
        EXPORTING
          clsname                   = ls_gen_cust-ad_bpm_dp_cl_obj
          with_inherited_components = abap_true
          with_interface_components = abap_true.

      lt_methods = lr_dp_class->get_methods( ).

      READ TABLE lt_methods TRANSPORTING NO FIELDS WITH KEY cmpname = /adesso/v_bpm_ob-dp_method.
      IF sy-subrc <> 0.
        MESSAGE e003(/adesso/bpm_utility) WITH ls_gen_cust-ad_bpm_dp_cl_obj.
      ELSE.
        RETURN.
