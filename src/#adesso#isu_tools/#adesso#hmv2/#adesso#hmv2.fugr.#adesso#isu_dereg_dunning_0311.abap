FUNCTION /adesso/isu_dereg_dunning_0311.
*"----------------------------------------------------------------------
...
*"----------------------------------------------------------------------
  ...
  DATA:
        t_hmv_cons TYPE TABLE OF /adesso/hmv_cons,
...
*    ------- do not process documents with origin 77 ---------*
      ...
      IF lt_fkkop IS NOT INITIAL.
        ...
        CALL FUNCTION '/ADESSO/FM_GET_MEMIDOC_TO_DUN'
          IMPORTING
        ...
        IF lt_memidoc IS NOT INITIAL.
          ...
          LOOP AT lt_fkkop ASSIGNING <fkkop>.
            LOOP AT lt_memidoc ASSIGNING <memidoc>
              ...
              SELECT COUNT( * ) FROM /adesso/mem_mloc WHERE doc_id = <memidoc>-doc_id AND fdate <= para_0300-ausdt AND tdate >= para_0300-ausdt AND lvorm = ' '.
              if sy-subrc <> 0.
