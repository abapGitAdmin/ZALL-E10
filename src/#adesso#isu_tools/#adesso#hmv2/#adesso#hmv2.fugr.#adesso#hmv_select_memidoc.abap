FUNCTION /adesso/hmv_select_memidoc.
*"----------------------------------------------------------------------
...
*"  TABLES
*"      IT_SELCT_MEMI STRUCTURE  /ADESSO/HMV_SELCT_MEMI
...
*"----------------------------------------------------------------------
  ...
  DATA ls_hmv_memi TYPE /adesso/hmv_memi.

  DATA: lt_mloc TYPE TABLE OF /adesso/mem_mloc.          "Nuss 02.02.2018
...
*-------------------- Pos. REMADV --------------------*
    ...
    WITH KEY own_invoice_no = wet_out-ownrf
               invoice_type = c_invoice_type7         "'007'.
         ."    BINARY SEARCH.
...
* IDOC-Status-Ermittlung----------------------------------------------------------------*
    ...
    SELECT SINGLE * FROM /adesso/hmv_memi INTO ls_hmv_memi WHERE doc_id = <s_memidoc>-doc_id.
