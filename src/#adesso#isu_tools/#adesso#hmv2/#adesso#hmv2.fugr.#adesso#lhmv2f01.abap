...
* New Version with Index TINV_INV_LINE_A OWN_INVOIVE_NO
  ...
  SORT t_remadv BY own_invoice_no invoice_type.
...
* --> Nuss 05.03.2018
  DATA: lv_hvorg        TYPE hvorg_kk,
        ...
        lt_akto         TYPE TABLE OF /adesso/hmv_akto WITH HEADER LINE,
        ls_akto         TYPE          /adesso/hmv_akto.

  SELECT * FROM /adesso/hmv_akto INTO TABLE lt_akto.
...
* Ausgeglichene MeMi-Belege berücksichtigen, wenn Ausgleichsstatus 9 selektiert
        ...
        SELECT *
          ...
          INTO ls_docstatus
          WHERE doc_status GE '80' OR doc_status EQ '65' OR doc_status EQ c_memidoc_dnlcrsn.
...
* nicht ausgeglichene MeMi-Belege berücksichtigen
        ...
        SELECT *
          FROM /idxmm/docstatus
          INTO ls_docstatus
          WHERE doc_status LT '80' AND doc_status NE '65' OR doc_status EQ c_memidoc_dnlcrsn.
...
* <<< ET_20160315
  ...
  IF c_idxmm_sp03_dunn IS INITIAL.
    ...
    INSERT INTO /adesso/mem_mloc VALUES ls_mloc.
...
*    ENDTRY.
    ...
    IF sy-subrc = 0.
      wet_out-doc_status = c_memidoc_dnlcrsn.
    ENDIF.
...
FORM vorschlag_mahnsperre CHANGING ls_out TYPE /adesso/hmv_out.
  DATA: ls_mloc TYPE /adesso/mem_mloc.
  DATA ls_memidoc_u TYPE /idxmm/memidoc.
...
**  Vorschläge für Mahnsperren gesetzt
  IF ls_out-kennz =   c_doc_kzd.                                                 "Nuss 02.02.2018
    IF c_mahnen IS INITIAL.                                                      "Nuss 06.2018
...
** --> Nuss 02.02.2018
    ENDIF.                                                                       "Nuss 06.2018
  ENDIF.
  IF ls_out-kennz = c_doc_kzm.
    IF c_mahnen_memi IS INITIAL.                                                 "Nuss 06.2018                                                                    "Nuss 06.2018
...
** --> Nuss 09.2018
  IF ls_out-kennz =   c_doc_kzmsb.
    IF c_mahnen_msb IS INITIAL.
      IF ls_out-payst_icon <> ' '. "icon_led_green OR lr_out->payst_icon = ' '.
