    ...
    LOOP AT t_tasks ASSIGNING <t_tasks>.
      CASE <t_tasks>-name(1).
        WHEN c_doc_kzd.
          ...
          ADD 1 TO x_runts.
          CALL FUNCTION '/ADESSO/HMV_SELECT'
...
* HMV_Select für MEMIDOC
        WHEN /idxmm/if_constants=>gc_createdfrom_m.
          ...
          ADD 1 TO x_runts.
          CALL FUNCTION '/ADESSO/HMV_SELECT_MEMIDOC'
            STARTING NEW TASK <t_tasks>-name
            DESTINATION IN GROUP DEFAULT
...
* -->  Nuss 09.2018
        WHEN c_doc_kzmsb.
          ...
          CALL FUNCTION '/ADESSO/HMV_SELECT_MSBDOC'
            STARTING NEW TASK <t_tasks>-name
            DESTINATION IN GROUP DEFAULT
...
FORM fieldcat_init USING lt_fieldcat TYPE slis_t_fieldcat_alv.
  ...
  CALL FUNCTION 'REUSE_ALV_FIELDCATALOG_MERGE'
    EXPORTING
      ...
      i_structure_name       = '/ADESSO/HMV_OUT'
...
*      ls_mloc-lvorm     = ''.
**        INSERT INTO /adesso/hmv_mloc VALUES ls_mloc.      "Nuss 02.02.2018
*      MODIFY /adesso/hmv_mloc FROM ls_mloc.              "Nuss 02.02.2018
*
*      IF sy-subrc = 0.
*        <t_out>-mansp  = pa_lockr.
*        <t_out>-fdate  = pa_fdate.
...
FORM ende_task USING taskname.
  RECEIVE RESULTS FROM FUNCTION '/ADESSO/HMV_SELECT'
    TABLES
         et_out      = ft_out
    EXCEPTIONS
       communication_failure = 1
...
FORM ende_task_memi USING taskname.
  RECEIVE RESULTS FROM FUNCTION '/ADESSO/HMV_SELECT_MEMIDOC'
    TABLES
         et_out      = ft_out
    EXCEPTIONS
       communication_failure = 1
...
FORM ende_task_msb USING taskname.
  RECEIVE RESULTS FROM FUNCTION '/ADESSO/HMV_SELECT_MSBDOC'
  TABLES
  et_out      = ft_out
  EXCEPTIONS
  communication_failure = 1
...
*    CALL FUNCTION '/ADESSO/HMV_MEMI_LOCKHIST'
*      EXPORTING
*        doc_id = wa_out-opbel.
    CALL FUNCTION '/ADESSO/MEMI_MAHNSPERRE'
      EXPORTING
...
FORM dunn_hist USING tabindex TYPE slis_selfield-tabindex.
  ...
  FIELD-SYMBOLS <l_out>  TYPE /adesso/hmv_out.
...
*** >>> ET_20160308
* Customizing Tabelle Intervallgrenzen für Vertragskonten (/adesso/hmv_ival)
  SELECT * FROM /adesso/hmv_ival INTO TABLE t_hmv_ival.
...
* Mahnverfahren
* --> Nuss 05.03.2018
  SELECT * FROM /adesso/hmv_mver INTO TABLE t_hmv_mver.
