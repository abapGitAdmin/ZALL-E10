FUNCTION /adesso/mtb_bel_move_in_h.
*"----------------------------------------------------------------------
...
* werden müssen (siehe: case itrans-dttyp).
      IF itrans-oldkey NE oldkey_moh AND
            oldkey_moh NE space.

*        oldver = oldkey_moh.

* Umschlüsselung-FUBA (aus Customizing Tabelle s.o.)
*        imoh_ever2[] = imoh_ever[].
*        ADD 1 TO vercount.
...
*  Dateiaufbereitung zum erstellen der Workbench-Dateien
        PERFORM aufbereitung_dat_moh USING oldkey_moh
*                                           vercount
*                                           veranz
                                           rep_name
                                           form_name.
        anz_obj = anz_obj + 1.
*       imoh_ever[] = imoh_ever2[].

        CLEAR: imoh_ever,imoh_ever2.
        REFRESH: imoh_ever, imoh_ever2.
...
* => je Datentyp eigene Tabelle
      CASE itrans-dttyp.
        WHEN 'EVER'.
          CLEAR x_imoh_ever.
          MOVE itrans-data TO x_imoh_ever.
          MOVE-CORRESPONDING x_imoh_ever TO imoh_ever.
          APPEND imoh_ever.
          CLEAR imoh_ever.
      ENDCASE.

      MOVE itrans-oldkey TO oldkey_moh.
...
* Erstellen der Migrationsdatei
  ...
  ELSE.
    PERFORM erst_mig_datei TABLES i_moh_down
                            USING firma
                                  object
                                  idttyp
                                  bel_file.
