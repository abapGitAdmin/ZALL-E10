FUNCTION /adesso/mtb_bel_strt_route.
*"----------------------------------------------------------------------
...
* werden müssen (siehe: case itrans-dttyp).
      IF itrans-oldkey NE oldkey_srt AND
            oldkey_srt NE space.

* Umschlüsselung-FUBA (aus Customizing Tabelle s.o.)
        IF NOT ums_fuba IS INITIAL.
*         CALL FUNCTION '/ADESSO/MTU_SAMPLE_BEL_STRT_ROU'
...
*  Dateiaufbereitung zum erstellen der Workbench-Dateien
        PERFORM aufbereitung_dat_srt USING oldkey_srt
                                           rep_name
                                           form_name.
        anz_obj = anz_obj + 1.
...
* => je Datentyp eigene Tabelle
      CASE itrans-dttyp.
        WHEN 'MRU'.
          MOVE itrans-data TO isrt_mru.
          APPEND isrt_mru.
          CLEAR isrt_mru.
        WHEN 'EQUNR'.
          MOVE itrans-data TO isrt_equnr.
          APPEND isrt_equnr.
          CLEAR isrt_equnr.
      ENDCASE.

      MOVE itrans-oldkey TO oldkey_srt.
...
* Erstellen der Migrationsdatei
  ...
  ELSE.
    PERFORM erst_mig_datei TABLES i_srt_down
                            USING firma
                                  object
                                  idttyp
                                  bel_file.
