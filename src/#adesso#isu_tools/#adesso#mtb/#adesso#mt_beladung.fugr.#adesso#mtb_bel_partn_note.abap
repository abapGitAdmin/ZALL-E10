FUNCTION /adesso/mtb_bel_partn_note.
*"----------------------------------------------------------------------
...
* werden müssen (siehe: case itrans-dttyp).
      IF itrans-oldkey NE oldkey_pno AND
            oldkey_pno NE space.

* Umschlüsselung-FUBA (aus Customizing Tabelle s.o.)
        IF NOT ums_fuba IS INITIAL.
*         CALL FUNCTION '/ADESSO/MTU_SAMPLE_BEL_PARTN_NO'
...
*  Dateiaufbereitung zum erstellen der Workbench-Dateien
        PERFORM aufbereitung_dat_pno USING oldkey_pno
                                           rep_name
                                           form_name.
        anz_obj = anz_obj + 1.
...
* => je Datentyp eigene Tabelle
      CASE itrans-dttyp.
        WHEN 'NOTKEY'.
          CLEAR x_i_notkey.
          MOVE itrans-data TO x_i_notkey.
          MOVE-CORRESPONDING x_i_notkey TO i_notkey.
          APPEND i_notkey.
          CLEAR i_notkey.
        WHEN 'NOTLIN'.
          CLEAR x_i_notlin.
          MOVE itrans-data TO x_i_notlin.
          MOVE-CORRESPONDING x_i_notlin TO i_notlin.
          APPEND i_notlin.
          CLEAR i_notlin.
      ENDCASE.

      MOVE itrans-oldkey TO oldkey_pno.
...
* Erstellen der Migrationsdatei
  ...
  ELSE.
    PERFORM erst_mig_datei TABLES i_pno_down
                            USING firma
                                  object
                                  idttyp
                                  bel_file.
