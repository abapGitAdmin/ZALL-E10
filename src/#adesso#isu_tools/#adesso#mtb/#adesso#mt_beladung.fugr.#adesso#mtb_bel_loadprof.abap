FUNCTION /adesso/mtb_bel_loadprof.
*"----------------------------------------------------------------------
...
* Daten werden um einen Altsystemschlüssel verzögert aufgebaut, weil
      IF itrans-oldkey NE oldkey_lop AND
            oldkey_lop NE space.
...
*  Dateiaufbereitung zum erstellen der Workbench-Dateien
        PERFORM aufbereitung_dat_lop USING oldkey_lop
                                           rep_name
                                           form_name.
        anz_obj = anz_obj + 1.
        ...
        CLEAR:  ilop_key[], ilop_key2[],
             ilop_elpass[], ilop_elpass2[].
      ENDIF.

      CASE itrans-dttyp.
        WHEN 'KEY'.
          CLEAR x_ilop_key.
          MOVE itrans-data TO x_ilop_key.
          MOVE-CORRESPONDING x_ilop_key TO ilop_key.
          APPEND ilop_key.
          CLEAR ilop_key.

        WHEN 'ELPASS'.
          CLEAR x_ilop_elpass.
          MOVE itrans-data TO x_ilop_elpass.
          MOVE-CORRESPONDING x_ilop_elpass TO ilop_elpass.
          APPEND ilop_elpass.
          CLEAR ilop_elpass.
      ENDCASE.

      MOVE itrans-oldkey TO oldkey_lop.
...
* Erstellen der Migrationsdatei
  ...
  ELSE.
    PERFORM erst_mig_datei TABLES i_lop_down
                            USING firma
                                  object
                                  idttyp
                                  bel_file.
