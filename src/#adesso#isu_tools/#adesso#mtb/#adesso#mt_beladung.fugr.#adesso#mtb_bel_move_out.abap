FUNCTION /adesso/mtb_bel_move_out.
*"----------------------------------------------------------------------
...
*   Migrationsfirma prüfen.
      ...
      IF itrans-oldkey NE oldkey_moo AND
            oldkey_moo NE space.
...
*         CALL FUNCTION '/ADESSO/MTU_SAMPLE_BEL_METERREA'
          CALL FUNCTION ums_fuba
            ...
            TABLES
              meldung     = meldung
              imoo_eausd  = imoo_eausd
              imoo_eausvd = imoo_eausvd
            CHANGING
              oldkey_moo  = oldkey_moo.
        ENDIF.

*  Dateiaufbereitung zum erstellen der Workbench-Dateien
        PERFORM aufbereitung_dat_moo USING oldkey_moo
                                           rep_name
                                           form_name.
        anz_obj = anz_obj + 1.
...
* => je Datentyp eigene Tabelle
      CASE itrans-dttyp.
        WHEN 'EAUSD'.
          MOVE itrans-data TO imoo_eausd.
          APPEND imoo_eausd.
          CLEAR imoo_eausd.
        WHEN 'EAUSVD'.
          MOVE itrans-data TO imoo_eausvd.
          APPEND imoo_eausvd.
          CLEAR imoo_eausvd.
      ENDCASE.

      MOVE itrans-oldkey TO oldkey_moo.
...
* Erstellen der Migrationsdatei
  ...
  ELSE.
    PERFORM erst_mig_datei TABLES i_moo_down
                            USING firma
                                  object
                                  idttyp
                                  bel_file.
