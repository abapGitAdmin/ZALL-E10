FUNCTION /adesso/mtb_bel_premise.
*"----------------------------------------------------------------------
...
* werden müssen (siehe: case itrans-dttyp).
      IF itrans-oldkey NE oldkey_pre AND
            oldkey_pre NE space..

* Umschlüsselung-FUBA (aus Customizing Tabelle s.o.)
        IF NOT ums_fuba IS INITIAL.
*         CALL FUNCTION '/ADESSO/MTU_SAMPLE_BEL_PREMISE'
...
*  Dateiaufbereitung zum erstellen der Workbench-Dateien
        PERFORM aufbereitung_dat_pre USING oldkey_pre
                                           rep_name
                                           form_name.
        anz_obj = anz_obj + 1.
...
* => je Datentyp eigene Tabelle
      CASE itrans-dttyp.
        WHEN 'EVBSD'.
          CLEAR x_i_evbsd.
          MOVE itrans-data TO x_i_evbsd.
          MOVE-CORRESPONDING x_i_evbsd TO i_evbsd.
          APPEND i_evbsd.
          CLEAR i_evbsd.
      ENDCASE.

      MOVE itrans-oldkey TO oldkey_pre.
...
* Erstellen der Migrationsdatei
  ...
  ELSE.
    PERFORM erst_mig_datei TABLES i_pre_down
                            USING firma
                                  object
                                  idttyp
                                  bel_file.
