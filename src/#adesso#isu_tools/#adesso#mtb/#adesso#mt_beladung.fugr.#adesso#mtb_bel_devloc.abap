FUNCTION /adesso/mtb_bel_devloc.
*"----------------------------------------------------------------------
...
* werden müssen (siehe: case itrans-dttyp).
      IF itrans-oldkey NE oldkey_dlc AND
            oldkey_dlc NE space.

* Umschlüsselung-FUBA (aus Customizing Tabelle s.o.)
        IF NOT ums_fuba IS INITIAL.
*         CALL FUNCTION '/ADESSO/MTU_SAMPLE_BEL_DEVLOC'
...
*  Dateiaufbereitung zum erstellen der Workbench-Dateien
        PERFORM aufbereitung_dat_dlc USING oldkey_dlc
                                           rep_name
                                           form_name.
        anz_obj = anz_obj + 1.
...
* => je Datentyp eigene Tabelle
      CASE itrans-dttyp.
        WHEN 'EGPLD'.
          CLEAR x_i_egpld.
          MOVE itrans-data TO x_i_egpld.
          MOVE-CORRESPONDING x_i_egpld TO i_egpld.
          APPEND i_egpld.
          CLEAR i_egpld.
      ENDCASE.

      MOVE itrans-oldkey TO oldkey_dlc.
...
* Erstellen der Migrationsdatei
  ...
  ELSE.
    PERFORM erst_mig_datei TABLES i_dlc_down
                            USING firma
                                  object
                                  idttyp
                                  bel_file.
