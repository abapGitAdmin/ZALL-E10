...
* werden müssen (siehe: case itrans-dttyp).
      IF itrans-oldkey NE oldkey_fac AND
            oldkey_fac NE space.

* Umschlüsselung-FUBA (aus Customizing Tabelle s.o.)
        IF NOT ums_fuba IS INITIAL.
*         CALL FUNCTION '/ADESSO/MTU_SAMPLE_BEL_FACTS'
...
*           Dateiaufbereitung zum erstellen der Workbench-Dateien
          PERFORM aufbereitung_dat_fac USING oldkey_fac
                                             rep_name
                                             form_name.
          anz_obj = anz_obj + 1.
...
* => je Datentyp eigene Tabelle
      CASE itrans-dttyp.
        WHEN 'KEY'.
          CLEAR x_ifac_key.
          MOVE itrans-data TO x_ifac_key.
          MOVE-CORRESPONDING x_ifac_key TO ifac_key.
          APPEND ifac_key.
          CLEAR ifac_key.

        WHEN 'FACTS'. "Datentyp Fakten aus eigener Struktur
          MOVE itrans-data TO ifac_facts.
      ...
      MOVE itrans-oldkey TO oldkey_fac.
...
* Erstellen der Migrationsdatei
  ...
  ELSE.
    PERFORM erst_mig_datei TABLES i_fac_down
                            USING firma
                                  object
                                  idttyp
                                  bel_file.
