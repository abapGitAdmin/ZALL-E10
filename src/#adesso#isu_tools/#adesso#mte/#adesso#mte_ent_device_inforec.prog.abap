*&---------------------------------------------------------------------*
*& Report  /ADESSO/MTE_ENT_DEVICE_INFOREC
*&
*&---------------------------------------------------------------------*
*&
*& Report zum Entladen von Geräteinfosatz-Daten aus Geräten
*&
*&---------------------------------------------------------------------*
REPORT /adesso/mte_ent_device_inforec.
...
PARAMETERS: firma LIKE temob-firma DEFAULT 'AEISU' OBLIGATORY.
...
PARAMETERS: exp_path LIKE temfd-path
    DEFAULT '/Mig/AE_BI/liefersystem/entladung/rwe/'.
SELECTION-SCREEN END OF LINE.
SELECTION-SCREEN BEGIN OF LINE.
...
* Achtung - aus Relevanztabelle holen wir Geräte
...
* Fuba Aufrufen
      CALL FUNCTION '/ADESSO/MTE_ENT_DEVICE_INFOREC'
        EXPORTING
          firma              =  firma
          x_equnr            =  i_equnr
...
*            WITH sy-msgv1 sy-msgv2 sy-msgv3 sy-msgv4.* MESSAGE ID SY-MSGID TYPE SY-MSGTY NUMBER SY-MSGNO
