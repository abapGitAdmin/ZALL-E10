...
************************************************************************
*******
REPORT zdr_sql_import_csv_header.
...
*  aufsplitten des Zeileninhalts
  ...
  AT FIRST.
    TRANSLATE <lv_row_string> TO UPPER CASE.
    SPLIT <lv_row_string> AT p_sep INTO TABLE lt_header_components.
...
*  Datensatz der CSV-Datei den Komponenten der Datenbanktabelle entsprechend zuteilen
  DO lv_row_width TIMES.
    lv_component_name = lt_header_components[ sy-index ].
    ASSIGN COMPONENT lv_component_name OF STRUCTURE <ls_istruc> TO <lv_istruc_comp>.
    IF sy-subrc <> 0.
    ...
    ELSE.
      TRY.
*          if p_tab = '/ADO/SQL_TEST' and lv_component_name = 'DATA_AND_TIME_OF_THE_CALL'.
...
*          endif.
          if p_tab = '/ADO/SQL_911' and lv_component_name = 'DATA_AND_TIME_OF_THE_CALL'.
            lv_component_value = |{ lv_component_value+6(4) }{ lv_component_value+3(2) }{ lv_component_value+0(2) }{ lv_component_value+11(2) }{ lv_component_value+14(2) }|.
          endif.
          if p_tab = '/ADO/SQL_SALEREC' and ( lv_component_name = 'ORDER_DATE' OR
                                               lv_component_name = 'SHIP_DATE' ).
            SPLIT lv_component_value AT '.' INTO DATA(lv_tmp1) DATA(lv_tmp2) DATA(lv_tmp3).
            if strlen( lv_tmp1 ) = 1.
              lv_tmp1 = |0{ lv_tmp1 }|.
            endif.
            if strlen( lv_tmp2 ) = 1.
            ...
            lv_component_value = |{ lv_tmp3 }{ lv_tmp2 }{ lv_tmp1 }|.
          endif.
          if p_tab = '/ADO/SQL_PSID' and ( lv_component_name = 'PARTICIPATION' OR
                                           lv_component_name = 'CITY' OR
                                           lv_component_name = 'COLLEGE' OR
                                           lv_component_name = 'HCOLLEGE' ).
            if lv_component_value CS 'yes'.
              lv_component_value = 'X'.
            else.
              lv_component_value = ' '.
            endif.
          endif.
          if p_tab = '/ADO/SQL_MOVIES' and lv_component_name = 'AWARDS'.
            if lv_component_value CS 'Yes'.
              lv_component_value = 'X'.
            else.
              lv_component_value = ' '.
          ...
          if p_tab = '/ADO/SQL_STAR' and lv_component_name = 'BIRTH'.
            lv_component_value = lv_component_value+0(4).
          endif.
