...
FORM pf_set_text_noinput.
  ...
  IF p_z16 = 'X'.
* hier muss natuerlich das Feld der anderen Gruppe geloescht werden.
    CLEAR: d_eui2[]. " Mann muss darauf achten welche Felder Tabellen sind.
    LOOP AT SCREEN.
      IF screen-group1 = 'Z16'.
        screen-input = 1.
        MODIFY SCREEN.
      ELSEIF screen-group1 = 'Z26'.
        screen-input = 0.
        MODIFY SCREEN.
      ENDIF.
    ENDLOOP.
...
FORM pf_message_screen.
  IF p_z16 = 'X'.
    IF d_eui IS INITIAL. " or p_pdate is Initial or p_sproc is INITIAL.
      MESSAGE 'Alte Aufgabe' TYPE 'I'.
    ENDIF.
  ELSE.
    if d_eui2 is INITIAL.
    MESSAGE 'Neue Aufgabe' TYPE 'I'.
    Endif.
  ENDIF.
ENDFORM.

*  LOOP AT SCREEN.
*      IF p_z16 EQ 'X'.  "EQ oder =
*      IF screen-group1 = 'Z16'.
*        screen-input = '1'.
*      ENDIF.
*      IF screen-group1 = 'Z26'.
*        screen-input = '0'.
*      ENDIF.
*
*      MODIFY SCREEN.
