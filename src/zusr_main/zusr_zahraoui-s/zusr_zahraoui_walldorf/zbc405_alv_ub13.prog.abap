...
CLASS lcl_handler DEFINITION.
  PUBLIC SECTION.
  "üb13 enthält Ereignisbehndlungsmethode
CLASS-DATA: gv_zeile type i.
    CLASS-METHODS:
...
*            et_index_rows =                  " Indizes der selektierten Zeilen
     ...
     IF lines( lt_row ) > 0.
       ...
       lv_percentage = lv_occupi / lv_capa * 100.
        ...
        MESSAGE lv_text TYPE 'I'.
        ELSE.
          MESSAGE 'Bitte markieren Sie mindestens 1 Zeile  '(wad) type 'I'.
     ENDIF.
      WHEN 'CARRIER_INFO' .
          READ TABLE gt_flights INTO gs_flight index gv_zeile.
          "brauchen nur fluggeselschaft des wegen nur select single
  ...
  METHOD on_context_menu_request.
    DATA:ls_colo_info TYPE lvc_s_col.
    go_alv->get_current_cell(
      IMPORTING
        e_row     =     gv_zeile             " Zeile auf Grid
       es_col_id =          ls_colo_info        " Spalten-Id
*        es_row_no =                  " Numerische Zeilen ID
    ).
    "Ü13
    CASE ls_colo_info-fieldname.
      WHEN 'CARRID'.
...
*INCLUDE zbc405_alv_ub5_status_0100o01.
  INCLUDE zalv_ub13_create_and_transfer.
